...
export class BufferGeometry {
  ...
  setIndex(index: number[] | BufferAttribute): this {
    if (index instanceof BufferAttribute) {
      ...
      const data = maxVal > 65535 ? new Uint32Array(index) : new Uint16Array(index)
  ...
  computeBoundingSphere(): void {
    ...
    const center = new Vec3((box.min.x + box.max.x) * 0.5, (box.min.y + box.max.y) * 0.5, (box.min.z + box.max.z) * 0.5)
  ...
  applyMatrix4(m: Mat4): this {
    ...
    if (tangentAttr) {
      ...
      for (let i = 0; i < tangentAttr.count; i++) {
        const tx = tangentAttr.getX(i),
          ty = tangentAttr.getY(i),
          tz = tangentAttr.getZ(i)
    ...
    for (const [name, srcAttr] of geometry.attributes) {
      ...
      for (let i = 0; i < srcAttr.count; i++) {
        ...
        for (let k = 0; k < itemSize; k++) {
          ;(dstAttr.data as Float32Array)[dstIdx * itemSize + k] = (srcAttr.data as Float32Array)[i * itemSize + k]
        }
      }
