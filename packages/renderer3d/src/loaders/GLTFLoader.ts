...
function makeTypedArrayView(
  ...
  switch (componentType) {
    ...
    default:   throw new Error(`GLTFLoader: unknown componentType ${componentType}`);
  }
}
...
function extractAccessorData(
  ...
  if (acc.bufferView !== undefined) {
    ...
    switch (acc.componentType) {
      case 5120: base = new Int8Array   (acc.count * elemCount); break;
      case 5121: base = new Uint8Array  (acc.count * elemCount); break;
      case 5122: base = new Int16Array  (acc.count * elemCount); break;
      case 5123: base = new Uint16Array (acc.count * elemCount); break;
      case 5125: base = new Uint32Array (acc.count * elemCount); break;
      default:   base = new Float32Array(acc.count * elemCount); break;
    }
  }
  ...
  if (acc.sparse) {
    ...
    const indices   = makeTypedArrayView(idxBuf, sp.indices.componentType, idxOffset, sp.count, 1) as Uint16Array | Uint32Array | Uint8Array;
    ...
    const valAligned = valOffset % compSize === 0
      ? valBuf
      : ((): ArrayBuffer => {
...
export class GLTFLoader {
    ...
    for (let i = 0; i < nodeCount; i++) {
      ...
      if (gNode.mesh !== undefined) {
        ...
        if (lightRef !== undefined) {
          const lightDefs = ((json.extensions?.['KHR_lights_punctual'] as KhrLightsPunctualExtension | undefined)?.lights) ?? [];
    ...
    for (const gMat of json.materials ?? []) {
      ...
      switch (gMat.alphaMode ?? 'OPAQUE') {
        ...
        case 'MASK':
          ...
          (mat as MeshStandardMaterial & { alphaCutoff?: number }).alphaCutoff =
  ...
  private _buildMesh(
    ...
    if (primitives.length === 1) {
      return this._buildPrimitive(
        json, buffers, primitives[0], materials, isSkinned, gMesh.weights ?? nodeWeights,
      );
    }
    ...
    for (const prim of primitives) {
      const child = this._buildPrimitive(
        json, buffers, prim, materials, isSkinned, gMesh.weights ?? nodeWeights,
      );
      group.add(child);
    }
    return group;
  ...
  private _buildPrimitive(
    ...
    if (prim.indices !== undefined) {
      ...
      if (acc.componentType === 5125) {
        // UNSIGNED_INT
        indexArray = idxData instanceof Uint32Array
          ? idxData
          : new Uint32Array(idxData);
      } else {
        // UNSIGNED_SHORT (or UNSIGNED_BYTE — promote to Uint16)
        indexArray = idxData instanceof Uint16Array
          ? idxData
          : new Uint16Array(idxData);
      }
    ...
    if (prim.targets && prim.targets.length > 0) {
      for (const [targetIndex, target] of prim.targets.entries()) {
        for (const [gltfKey, accIdx] of Object.entries(target)) {
          const mapKey = gltfKey === 'POSITION' ? 'position'
                       : gltfKey === 'NORMAL'   ? 'normal'
                       : gltfKey === 'TANGENT'  ? 'tangent'
                       : gltfKey.toLowerCase();
          const data  = extractAccessorData(json, buffers, accIdx);
          const acc   = json.accessors![accIdx];
    ...
    const mat: MeshStandardMaterial | MeshBasicMaterial =
      prim.material !== undefined
        ...
        : new MeshStandardMaterial('default');
  ...
  private _buildPunctualLight(lightDef?: KhrLight): Object3D {
    ...
    const color = lightDef.color
      ? new Vec3(lightDef.color[0], lightDef.color[1], lightDef.color[2])
      : new Vec3(1, 1, 1);
  ...
  private _parseAnimations(
    ...
    for (let ai = 0; ai < (json.animations ?? []).length; ai++) {
      ...
      for (const channel of gAnim.channels) {
        ...
        const values = valRaw instanceof Float32Array
          ? valRaw
          : new Float32Array(valRaw);
